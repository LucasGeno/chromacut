...
```bash
# 1. Mint a short-lived (1-day) session token on the droplet.
TOKEN=$(ssh platform "docker exec -w /app admin-panel python -c \
  \"from app.db.sessions import create_session; \
    print(create_session('lucas.reed2000@gmail.com','admin',1))\"")

# 2. Drive the authenticated flow against prod.
SESSION_TOKEN="$TOKEN" node scripts/e2e-signed-in.mjs

# 3. ALWAYS clean up — delete the minted session.
ssh platform "docker exec -w /app admin-panel python -c \
  \"from app.db.sessions import delete_session; delete_session('$TOKEN')\""
```
